...
function Team(props) {
  return (
    <div>
      ...
      <p>Score: {getTeamScore(props.team)}</p>
      <p>Players: {props.team.players.length}</p>
    </div>
  );
}
...
export function TeamsList() {
  ...
  function orderTeamByScoreHighestToLowest() {
    // avoid no re-rendering because sort does not return a new array (returns the same)
    const teamsCopy = [...teams];

    const newTeams = teamsCopy.sort(
      (team1, team2) => getTeamScore(team2) - getTeamScore(team1)
    );
  ...
  function orderTeamByScoreLowestToHighest() {
    ...
    const newTeams = teamsCopy
      .sort((team1, team2) => getTeamScore(team2) - getTeamScore(team1))
      .reverse();
  ...
  function teamsWithMoreThanThreePlayers() {
    const newTeams = teams.filter((team) => team.players.length > 3);
    setTeams(newTeams);
  }
